...
URGENCY_LEVELS = [
    ...
    "Prompt appointment",
    "Monitor and discuss if persistent",
]
...
Choose exactly one urgency level from the supplied list. Do not use urgency as a substitute for the report:
give clinically useful detail in every section. Return only JSON conforming to the schema.
...
def generate_long_report(patient: dict, progress_callback=None) -> dict:
    ...
    urgency_instructions = (
        "You are a careful clinical-information assistant. Based on the intake below, choose exactly "
        "one urgency level from the supplied list. If a possible emergency is plausible, choose "
        '"Emergency now". Return only JSON conforming to the schema — no explanation.'
    )
...
with st.sidebar:
    ...
    st.caption(
        f"Locally: put it in {ENV_PATH.name} (same folder as this script, never committed to git). "
        "On Streamlit Community Cloud: app Settings -> Secrets, as GROQ_API_KEY = \"...\"."
    )
    st.caption("Records are stored locally in patient_records.db beside this app.")
    with st.expander("Debug: where is premed.py looking for opai.env?"):
        st.code(str(ENV_PATH), language=None)
        st.write("File found at that path:", ENV_PATH.exists())
        if ENV_PATH.exists():
            found_key = "GROQ_API_KEY=" in ENV_PATH.read_text(encoding="utf-8")
            st.write("Contains a GROQ_API_KEY= line:", found_key)
...
with st.form("health_intake"):
    ...
    with second:
        severity = st.slider("Severity now (1 mild — 10 worst)", 1, 10, 5)
    ...
    long_report = st.checkbox(
        "Generate a longer report (takes a few minutes — several timed requests, spaced out to fit "
        "your Groq plan's per-minute limit, instead of one shortened request)",
        value=True,
    )
